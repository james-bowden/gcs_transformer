...
class UavEnvironment:
    ...
    def __init__(
        ...
        DEFAULT_START = (0.0, 0.0, 2.0), DEFAULT_GOAL = (15.0, 15.0, 0.5)
    ) -> None:
        ...
        # Channels are: [CellType, Top asset, Bottom asset, Left asset, Right asset, Center asset]
        self.map_array = np.zeros([6, self.shape[0], self.shape[1]], dtype=int)
        ...
        self.grid[round(self.DEFAULT_START[0] / CELL_SIZE), round(self.DEFAULT_START[1] / CELL_SIZE)] = CellType.OUTDOOR
        self.grid[round(self.DEFAULT_GOAL[0] / CELL_SIZE), round(self.DEFAULT_GOAL[1] / CELL_SIZE)] = CellType.INDOOR
        ...
        for xp in range(self.grid.shape[0]):
            for yp in range(self.grid.shape[1]):
                grid_cell_type = self.grid[xp, yp]
                self.map_array[0, xp, yp] = grid_cell_type.value

        self.map_array[0, round(self.DEFAULT_START[0] / CELL_SIZE), round(self.DEFAULT_START[1] / CELL_SIZE)] = 0 # Mark start
        self.map_array[0, round(self.DEFAULT_GOAL[0] / CELL_SIZE), round(self.DEFAULT_GOAL[1] / CELL_SIZE)] = 3 # Mark goal
        ...
        # Map from asset creation fxn to int
        self.asset_to_index = [
            Building.make_internal_no_wall,
            Building.make_internal_horizontal_wall_right,
            Building.make_internal_horizontal_wall_left,
            Building.make_internal_vertical_wall,
            Building.make_internal_door,
            Building.make_external_windows,
            Building.make_external_window_right,
            Building.make_external_window_left,
            Building.make_external_door,
            Building.make_external_wall,
            OutdoorDecoration.make_nothing,
            OutdoorDecoration.make_tree,
        ]
        self.asset_to_index = {
            k: v for v, k in enumerate(self.asset_to_index)
        }
            ...
            for y_idx in range(self.grid.shape[1]):
                ...
                match self.grid[x_idx, y_idx]:
                    case CellType.INDOOR:
                        ...
                        for direction, (
                            ...
                            if (
                                ...
                                # Add indoor wall to list
                                if outdoor_building_asset_creation_function not in self.walls.keys():
                                    self.walls[outdoor_building_asset_creation_function] = []
                                self.walls[outdoor_building_asset_creation_function].append((p_Wo_wall[0], p_Wo_wall[1], direction))
                                self.map_array[1 + direction.value, x_idx, y_idx] = self.asset_to_index[outdoor_building_asset_creation_function]
                            ...
                            if self.grid[
                                ...
                                # Add indoor wall to list
                                if indoor_asset_creation_function not in self.walls.keys():
                                    self.walls[indoor_asset_creation_function] = []
                                self.walls[indoor_asset_creation_function].append((p_Wo_wall[0], p_Wo_wall[1], direction))
                                self.map_array[1 + direction.value, x_idx, y_idx] = self.asset_to_index[indoor_asset_creation_function]
                    ...
                    case CellType.OUTDOOR:
                        ...
                        # Add outdoor assets (trees or nothing). This is always in the "center" channel --
                        # since it's not a wall, it has no direction
                        self.map_array[-1, x_idx, y_idx] = self.asset_to_index[outdoor_asset_creation_function]
